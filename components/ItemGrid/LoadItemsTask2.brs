...
sub init()
    m.log = log_Logger("LoadItemsTask2")
    ...
    if data <> invalid
        ...
        for each item in data.Items
            ...
            if item.Type = "Movie" or item.Type = "MusicVideo"
                ...
                tmp.title = item.name
                genreData = api_users_GetItemsByQuery(m.global.session.user.id, {
                    SortBy: "Random"
                    SortOrder: "Ascending"
                    IncludeItemTypes: m.top.itemType
                    Recursive: true
                ...
                if genreData.Items.Count() > 5
                    ...
                    if LCase(m.top.itemType) = "movie"
                        genreItemImage = api_items_GetImageURL(item.id)
                    else
                ...
                for each genreItem in genreData.Items
                    ...
                    genreItemImage = api_items_GetImageURL(genreItem.id)
