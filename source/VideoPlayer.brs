...
function PlayIntroVideo(video_id, audio_stream_idx) as boolean
    ...
    if m.global.session.user.settings["playback.cinemamode"] = true
        ...
        if introVideos = invalid then
            return true
        end if
        if introVideos.TotalRecordCount > 0

            if lcase(introVideos.items[0].name) = "rick roll'd" then
                return true
            end if
...
function getContainerType(meta as object) as string
    ...
    if not IsValid(meta.json) or not isValid(meta.json.mediaSources) then
        return ""
    end if
...
function getAudioFormat(meta as object) as string
    ...
    if audioInfo.count() = 0 or audioInfo[0].codec = invalid then
        return ""
    end if
